...
class connection(object):
    def __init__(self, client_socket, logger):
        ...
        self.conn_info = client_socket.getpeername()
        self.client_name = str(self.conn_info)
        self.client_ip = self.conn_info[0]
        self.client_port = int(self.conn_info[1])
    ...
    def log(self, timestamp, who, what):
        ...
        with open(txtfilename, 'a') as txtmitmfile:
            with open(binfilename, 'ab') as binmitmfile:
                jsondata = json.dumps({"timestamp": f"{time.time()}", "from": f"{who}", "data": f"{what}"})
